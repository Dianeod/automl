...
proc.runmodels:{[data;tgt;mdls;cnms;p;dt;fpath]
  ...
  if[`nlppretrain~p`typ;[knlp:first key[s1]where key[s1] in i.nlplist;knorm:first key[s1]where not key[s1] in i.nlplist;
    ...
    [select from mdls where model in(knorm;knlp)];
 ...
 preds:$[b:bs~`Combination;(scf[;ytst]proc.i.imax each avg probpr[;0];
        (probpr:proc.mdls[;xtrn;ytrn;xtst;ytst;mdls;p;0b]each knorm,knlp)[;1])
        ;(scf[;ytst]pr[0];(pr:proc.mdls[bs;xtrn;ytrn;xtst;ytst;mdls;p;1b])1)];
  // Get appropriate data columns for nlp or normal models
 if[not b;tt[`xtrain]:(tt`xtrain)[;inds:where $[bs in i.nlplist;;not]10h=type each first tt`xtrain];
    tt[`xtest]:(tt`xtest)[;inds];cnms:cnms[inds]];
  ...
  if[p[`saveopt]~2;$[b;post.combfeatureimpact[bs:knorm,knlp;(bm:preds 1;mdls);value tt;cnms;scf;dt;fpath;p];
